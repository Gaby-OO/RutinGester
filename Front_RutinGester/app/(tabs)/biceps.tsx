...
const TEMPLATE_BICEPS = EXERCISES.filter(e => e.group === 'biceps');

export default function BicepsRoutineScreen() {
  const { createRoutineFromTemplate } = useRoutine();
  const handleUseTemplate = async () => {
    const routine = await createRoutineFromTemplate('Plantilla Bíceps', TEMPLATE_BICEPS);
    if (routine) {
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <ThemedText type="title">Bíceps</ThemedText>
      <ThemedText style={styles.paragraph}>Una base sólida para trabajar bíceps con variaciones de curl. Usa la plantilla y modifícala según tu nivel.</ThemedText>
      <ThemedText style={[styles.paragraph, { opacity: 0.8 }]}>Ejercicios incluidos inicialmente:</ThemedText>
      <View style={styles.grid}>
        {TEMPLATE_BICEPS.map(e => (
