...
const TEMPLATE_QUADS = EXERCISES.filter(e => e.group === 'quadriceps');

export default function QuadricepsRoutineScreen() {
  const { createRoutineFromTemplate } = useRoutine();
  const handleUseTemplate = async () => {
    const routine = await createRoutineFromTemplate('Plantilla Cuádriceps', TEMPLATE_QUADS);
    if (routine) {
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <ThemedText type="title">Cuádriceps</ThemedText>
      <ThemedText style={styles.paragraph}>Plantilla centrada en el desarrollo de cuádriceps con sentadillas y extensiones. Ajusta volumen a tu objetivo.</ThemedText>
      <ThemedText style={[styles.paragraph, { opacity: 0.8 }]}>Ejercicios incluidos inicialmente:</ThemedText>
      <View style={styles.grid}>
        {TEMPLATE_QUADS.map(e => (
...
const styles = StyleSheet.create({
  ...
  exerciseTitle: { padding: 8, fontWeight: '600' },
  useButton: {
    marginTop: 16,
    flexDirection: 'row',
    alignItems: 'center',
  ...
  useButtonText: { color: '#fff', fontWeight: '700', fontSize: 16 },
});
