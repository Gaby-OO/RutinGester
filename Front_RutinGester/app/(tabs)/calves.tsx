...
const TEMPLATE_CALVES = EXERCISES.filter(e => e.group === 'calves');

export default function CalvesRoutineScreen() {
  const { createRoutineFromTemplate } = useRoutine();
  const handleUseTemplate = async () => {
    const routine = await createRoutineFromTemplate('Plantilla Gemelos', TEMPLATE_CALVES);
    if (routine) {
  ...
  return (
    <ThemedView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <ThemedText type="title">Gemelos</ThemedText>
        <ThemedText style={styles.paragraph}>Plantilla enfocada en sóleo y gastrocnemios con elevaciones de talón de pie y sentado.</ThemedText>
        <ThemedText style={[styles.paragraph, { opacity: 0.8 }]}>Ejercicios incluidos inicialmente:</ThemedText>
        <View style={styles.grid}>
          {TEMPLATE_CALVES.map(e => (
