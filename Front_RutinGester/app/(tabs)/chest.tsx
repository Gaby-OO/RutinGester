...
const TEMPLATE_CHEST = EXERCISES.filter(e => e.group === 'chest');

export default function ChestRoutineScreen() {
  const { createRoutineFromTemplate } = useRoutine();
  const handleUseTemplate = async () => {
    const routine = await createRoutineFromTemplate('Plantilla Pectorales', TEMPLATE_CHEST);
    if (routine) {
      router.replace({ pathname: '/(tabs)/routine/[id]', params: { id: routine.id.toString() } });
    }
    // Opcional: manejar el caso en que routine es null (el guardado falló)
  };
  return (
    <ThemedView style={styles.container}>
      ...
      <ThemedText type="title">Pectorales</ThemedText>
      <ThemedText style={styles.paragraph}>Esta plantilla se enfoca en el desarrollo del pecho con ejercicios compuestos y de peso corporal. Úsala como base y edítala a tu gusto.</ThemedText>
      <ThemedText style={[styles.paragraph, { opacity: 0.8 }]}>Ejercicios incluidos inicialmente:</ThemedText>
      <View style={styles.grid}>
        {TEMPLATE_CHEST.map(e => (
